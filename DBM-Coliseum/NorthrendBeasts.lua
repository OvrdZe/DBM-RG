...
local timerCombatStart		= mod:NewCombatTimer(22.5) --|23|
local timerNextBoss			= mod:NewTimer(190, "TimerNextBoss", 2457, nil, nil, 1) --|150| 2min 30s, as per TC.
...
local timerImpaleCD			= mod:NewCDTimer(10, 66331, nil, "Tank|Healer", nil, 5, nil, DBM_COMMON_L.TANK_ICON, true) --|8| 2s variance [8.0-9.9]. Added "keep" arg (10H 2021/10/22 || 10N 2021/10/22 || 25H Lordaeron 2022/09/03) - 8.3, 8.6, 9.4, 8.0, 9.6, 9.4, 9.0, 9.1, 8.7, 9.4, 9.9, 8.2, 8.6 || 8.6, 8.1, 9.5, 9.6, 9.8, 8.7, 8.8, 9.6 || 9.2, 9.5, 9.7, 9.3, 9.5, 8.3, 8.5
...
local timerEmerge			= mod:NewNextSourceTimer(10, 66947, nil, nil, nil, 6, "Interface\\AddOns\\DBM-Core\\textures\\CryptFiendUnBurrow.blp") --|5|
...
local timerMoltenSpewCD		= mod:NewCDTimer(21, 66820, nil, "Tank", 2, 5, nil, DBM_COMMON_L.TANK_ICON, true) --|16.1| REVIEW! variance? Added "Keep" arg (25H Lordaeron 2022/09/28 || ) - 19.1 || 16.1
local timerParalyticSprayCD	= mod:NewCDTimer(21, 66901, nil, nil, nil, 3, nil, nil, true) --|16.2| REVIEW! ~11s variance? Added "Keep" arg (25H Lordaeron 2022/09/28 || 25H Lordaeron 2022/10/09 || 25N Lordaeron 2022/10/21) - 27.8 || 16.2 || 26.1
local timerBurningSprayCD	= mod:NewCDTimer(21, 66902, nil, nil, nil, 3, nil, nil, true) --|19| REVIEW! 5s variance? (25H Lordaeron 2022/09/03 || 25H Lordaeron 2022/09/28) - 20.6, 19.0 || 24.7
...
local timerNextCrash		= mod:NewCDTimer(51, 66683, nil, nil, nil, 2, nil, DBM_COMMON_L.MYTHIC_ICON) --|63.4| REVIEW! variance? (25H Lordaeron 2022/09/03) - 63.4, 63.7
...
function mod:OnCombatStart(delay)
	...
	self:SetStage(1)
	specWarnStompPreWarn:Schedule(14-delay) --|12| 3s pre-warn. (10N Lordaeron 2022/10/02) - 14.9
	if self.Options.soundConcAuraMastery and isBuffOwner("player", 19746) then -- Concentration Aura Mastery by a Paladin will negate the interrupt effect of Staggering Stomp
		soundAuraMastery:Schedule(14-delay, "Interface\\AddOns\\DBM-Core\\sounds\\PlayerAbilities\\AuraMastery.ogg") --|14|
	else
		specWarnStompPreWarn:ScheduleVoice(14-delay, "silencesoon") --|12|
	end
	if self:IsHeroic() then
		timerNextBoss:Start(152-delay) --|0|
		timerNextBoss:Schedule(147) --|copied from old file|
	end
	timerRisingAnger:Start(25-delay) --|18| REVIEW! ~10s variance? (25H Lordaeron 2022/09/03 || 25N Lordaeron 2022/09/23 || 25H Lordaeron 2022/09/24 || 25H Lordaeron 2022/09/28 || 10N Lordaeron 2022/10/02) - 18.9 || 26.1 || 29.7 || 21.9 || 18.2
...
function mod:SPELL_AURA_APPLIED_DOSE(args)
	...
	elseif args.spellId == 66636 then	-- Rising Anger
		...
		if amount < 3 then
--			if self:IsHeroic() then
				timerRisingAnger:Start(15) --|17.5| (25H Lordaeron 2022/09/28) - 17.5
--			else
--				if amount < 3 then
...
--		self:ScheduleMethod(13.5, "WormsEmerge")
		timerCombatStart:Start(10) --|13.5|
...
--		self:UnscheduleMethod("WormsEmerge")
		timerCombatStart:Start(9.5) --|10|
		timerNextCrash:Start(40) --|copied from old DBM file|
		timerNextBoss:Cancel()
	...
	for i = 1, 5 do
		...
		if UnitExists(unitID) then
			...
			if cid == 35144 and not acidmawEngaged then -- Acidmaw (stationary on engage)
				...
				timerSubmerge:Start(45, bossName) --|49.3| REVIEW! 2s delay from visual to submerge (25H Lordaeron 2022/09/03) - 50
				timerSweepCD:Start(16, bossName) --|18.6| (2021/10/21 || 25H Lordaeron 2022/09/03 || 10N Lordaeron 2022/10/02 || 25H Lordaeron 2022/10/09) - 22-24s (N/H?) || 24 || 20.0 || 18.6
			...
			elseif cid == 34799 and not dreadscaleEngaged then -- Dreadscale (mobile on engage)
				...
				timerSlimePoolCD:Start(14, bossName) --|15| (25H Lordaeron 2022/09/03) - 15
				timerMoltenSpewCD:Start(16) --|19.2| (25H Lordaeron 2022/09/03 || 25H Lordaeron 2022/09/28 || 25N Lordaeron 2022/10/13) - 24 || 23.2 || 19.2
				timerBurningBiteCD:Start(5) --|15.4| (25H Lordaeron 2022/09/03 || 25N Lordaeron 2022/10/13) - 19 || 15.4
			elseif cid == 34797 then -- Icehowl
				self:SetStage(3)
				timerBreathCD:Start()
				timerNextCrash:Start(59) --|40.9| REVIEW!
	...
	elseif spellName == GetSpellInfo(66947) then -- Emerge
		...
		if npcId == 35144 and acidmawSubmerged then -- Acidmaw
			...
			if self.vb.AcidmawMobile then
				...
				timerParalyticSprayCD:Start(18)	-- (old log review (N/H?) || 25H Lordaeron 2022/09/28) - 18-20 || 16.7
			end
		elseif npcId == 34799 then -- Dreadscale
			self.vb.DreadscaleMobile = not self.vb.DreadscaleMobile
			...
			if self.vb.DreadscaleMobile then
				...
				timerSweepCD:Start(17, dreadscale) -- (25H Lordaeron 2022/09/03 || 25N Lordaeron 2022/10/21) - 17 || 14.8
				timerBurningSprayCD:Start(20) -- (25H Lordaeron 2022/09/03 || 25H Lordaeron 2022/09/28) - 20 || 13.5
			end
		end
	end
end
