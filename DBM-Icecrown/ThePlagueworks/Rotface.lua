...
local timerStickyOoze			= mod:NewNextTimer(16, 69774, nil, "Tank", nil, 5, nil, DBM_COMMON_L.TANK_ICON)--|15|
local timerWallSlime			= mod:NewNextTimer(20, 69789) --|25| Edited.
local timerSlimeSpray			= mod:NewNextTimer(23, 69508, nil, nil, nil, 3) --|20| Some instances, timer is not fixed (cause unknown, but self-corrected on the next timer). (25H Lordaeron 2022/07/09 || 10N Icecrown 2022/08/25 || 25H Lordaeron [2023-08-23]@[20:45:25]) - 20.1, 20.0, 20.0, 20.1, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0 || 20.0, 20.1, 20.0, 20.0 || pull:20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.1, 20.0, 21.4, 18.6, 20.0, 20.0
...
function mod:OnCombatStart(delay)
	timerWallSlime:Start(25-delay) --|9| Adjust from 25 to 9 to have a correct timer from the start
	timerSlimeSpray:Start(15-delay) --|20| Custom add for the first Slime Spray. Log reviewed (25H Lordaeron 2022/07/09) - 20.0
	if self:IsHeroic() then
		timerVileGasCD:Start(28-delay) --|28.9| Edited. REVIEW! variance? (25H Lordaeron 2022/07/09 || 25H Lordaeron 2022/09/23) - 28.9 || 29.1
	end
